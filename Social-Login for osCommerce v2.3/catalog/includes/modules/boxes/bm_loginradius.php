...
// Starting loginradius login process.
  ...
  function keys() {
       return array('MODULE_BOXES_LOGINRADIUS_STATUS', 
                    ...
                    'MODULE_BOXES_LOGINRADIUS_EMAIL_REQUIRED',
					'MODULE_SOCIAL_LOGIN_USEAPI',
					'MODULE_SOCIAL_LOGIN_LINKACCOUNT'
              );
    }
 ...
 */
  function sociallogin_add_newUser($lrdata) {
    // Defining global variables. 
      global $oscTemplate, $cart, $navigation, $messageStack, $breadcrumb, $customer_id, $customer_first_name, $customer_default_address_id, $customer_country_id, $customer_zone_id, $password, $confirm, $customer_picture;
  ...
  if (isset($lrdata['id']) && !empty($lrdata['id']) && !empty($lrdata['Email'])) {
	 ...
	 // Getting default country id.
	  ...
	  // Getting default postcode id.
      $postcode_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'ENTRY_POSTCODE_MIN_LENGTH'");
      $postcode_array = tep_db_fetch_array($postcode_query);
      $postcode_id = $postcode_array['configuration_value'];
 ...
 */
  function sociallogin_get_existUser($lrdata) {
    $check_existId = tep_db_query("select customers_id, customers_firstname, customers_password, customer_social_avatar, customers_email_address, customers_default_address_id from " . TABLE_CUSTOMERS . " where loginradiusid = '" . $lrdata['id'] . "'");
...
class LoginRadius {
  ...
  /*public function loginradius_get_auth($ApiKey, $ApiSecrete){
    ...
    if (isset($ApiKey)) {
      ...
      $ValidateUrl = "https://devhub.loginradius.com/getappinfo/$ApiKey/$ApiSecrete";
	  $JsonResponse = $this->loginradius_call_api($ValidateUrl);
