...
if(!tep_session_is_registered('customer_id')){
	  ...
	  tep_session_unregister('customer_picture');
	  tep_session_unregister('customer_social_provider');
}
  ...
  //$process = true;
   ...
   if (empty($lrdata['Email']) && $lrsetting['dummyemail'] == 1) {
	 ...
	 // Check if email exists
		$check_email_query = tep_db_query("SELECT * FROM " . TABLE_CUSTOMERS . " WHERE customers_email_address = '" . $lrdata['Email'] . "'");
		$check_email_array = tep_db_fetch_array($check_email_query);
		$customers_id=$check_email_array['customers_id'];
			if($customers_id){
...
elseif (isset($_POST['LoginRadiusEmailClick']) && !empty($_POST['session'])){
	...
	$query = tep_db_query("select configuration_title, configuration_key, configuration_value, configuration_description from " . TABLE_CONFIGURATION . " where configuration_key = '" . mysql_real_escape_string($_POST['session']) . "'");
		...
		if((isset($_POST['gender']) && empty($_POST['gender']))||(isset($_POST['firstname']) && empty($_POST['firstname']))||(isset($_POST['lastname']) && empty($_POST['lastname']))||(isset($_POST['dob']) && empty($_POST['dob']))||(isset($_POST['street_address']) && empty($_POST['street_address']))||(isset($_POST['email_address']) && empty($_POST['email_address']))||(isset($_POST['city']) && empty($_POST['city']))||(isset($_POST['state']) && empty($_POST['state']))||(isset($_POST['postcode']) && empty($_POST['postcode']))||(isset($_POST['telephone']) && empty($_POST['telephone']))){echo $msg .= 'Please fill all the fields<br>'; $lrerror .= 'lremptyerror ';}
		
	else{	
		  if (isset($_POST['street_address']) && strlen($_POST['street_address']) < ENTRY_STREET_ADDRESS_MIN_LENGTH) {
      ...
      $zone_id = 0;
      $check_query = tep_db_query("select count(*) as total from " . TABLE_ZONES . " where zone_country_id = '" . (int)$_POST['country'] . "'");
      $check = tep_db_fetch_array($check_query);
      $entry_state_has_zones = ($check['total'] > 0);
      if ($entry_state_has_zones == true) {
        $zone_query = tep_db_query("select distinct zone_id from " . TABLE_ZONES . " where zone_country_id = '" . (int)$_POST['country'] . "' and (zone_name = '" . tep_db_input($_POST['state']) . "' or zone_code = '" . tep_db_input($_POST['state']) . "')");
        if (tep_db_num_rows($zone_query) == 1) {
          $zone = tep_db_fetch_array($zone_query);
          $zone_id = $zone['zone_id'];
        } else {
          $msg .= ENTRY_STATE_ERROR_SELECT. "<br>";
			$lrerror .= 'lrstateerror ';
        }
      } else {
        if (strlen($_POST['state']) < ENTRY_STATE_MIN_LENGTH) {
         $msg .= ENTRY_STATE_ERROR. "<br>";
			$lrerror .= 'lrstateerror ';
        }
      }
	 ...
	 $check_email_query = "select * from " . TABLE_CUSTOMERS . " where customers_email_address = '" . tep_db_input($_POST['email_address']) . "'";
	...
	if(empty($lrdata['Email'])){
		$lrdata['Email'] = mysql_real_escape_string($_POST['email_address']);
	}	
	...
	if(empty($lrdata['address'])){
		$lrdata['address'] = mysql_real_escape_string($_POST['street_address']);
	}
	...
	if(($lrsetting['allfields']==1)&&((empty($lrdata['postcode']))||(empty($lrdata['Email']))||(empty($lrdata['FirstName']))||(empty($lrdata['LastName']))||(empty($lrdata['gender']))||(empty($lrdata['dob']))||(empty($lrdata['address']))||(empty($lrdata['city']))||(empty($lrdata['state']))||(empty($lrdata['telephone'])))){$msg .= $lrsetting['popupfieldmessage']; $lrerror .= 'lrdeleteposterror ';}
  ...
  function sociallogin_logging_existUser($lrdata) {
    ...
    // Defining global variables. 
	  ...
	  //sql quire for fatch user profile data from data base
		...
		if(!$check_customer){
			$check_email_query = tep_db_query("SELECT * FROM " . TABLE_CUSTOMERS . " WHERE customers_email_address = '" . $lrdata['Email'] . "'");
			$check_customer = tep_db_fetch_array($check_email_query);
		}
	 ...
	 tep_db_query("update " . TABLE_CUSTOMERS_INFO . " set customers_info_date_account_last_modified = now() where customers_info_id = '" . (int)$customer_id . "'");
      ...
      // restore cart contents
		...
		elseif(!empty($lrsetting['redirect'])){
...
function validateForm()
{
	...
	var loginRadiusForm = document.getElementById('loginRadiusForm');
		...
		for(var i = 0; i < loginRadiusForm.elements.length; i++){
			...
			if(loginRadiusForm.elements[i].name == "email_address"){
...
<?php
   ...
   $output .= '</div><form id="loginRadiusForm" onsubmit="return validateForm();" method="post" action=""><div align="right"><span style="padding-right:30px" id="errormassages">* Required information</span></div><table>';
   if($lrsetting['allfields'] != '0'){
   if(empty($lrdata['gender'])){
    $output .= '<tr><td><b id="textmatter">Gender</b></td><td>'.tep_draw_radio_field('gender', 'm') . '&nbsp;&nbsp;' . MALE . '&nbsp;&nbsp;' . tep_draw_radio_field('gender', 'f') . '&nbsp;&nbsp;' . FEMALE . '&nbsp;' . (tep_not_null(ENTRY_GENDER_TEXT) ? '<span class="inputRequirement">' . ENTRY_GENDER_TEXT . '</span>': '').'</td></tr>';
   }
   if(empty($lrdata['FirstName'])){
   $output .= '<tr><td><b id="textmatter">First Name</b></td><td>'.tep_draw_input_field('firstname') . '&nbsp;' . (tep_not_null(ENTRY_FIRST_NAME_TEXT) ? '<span class="inputRequirement">' . ENTRY_FIRST_NAME_TEXT . '</span>': '').'</td></tr>';
   }
   if(empty($lrdata['LastName'])){
   $output .= '<tr><td><b id="textmatter">Last Name</b></td><td>'.tep_draw_input_field('lastname') . '&nbsp;' . (tep_not_null(ENTRY_LAST_NAME_TEXT) ? '<span class="inputRequirement">' . ENTRY_LAST_NAME_TEXT . '</span>': '').'</td></tr>';
   }
   if(empty($lrdata['dob'])){
   $output .= '<tr><td><b id="textmatter">Date of Birth</b></td><td>'.tep_draw_input_field('dob') . '&nbsp;' . (tep_not_null(ENTRY_DATE_OF_BIRTH_TEXT) ? '<span class="inputRequirement">' . ENTRY_DATE_OF_BIRTH_TEXT . '</span>': '').'</td></tr>';
   }
   if(empty($lrdata['address'])){
   $output .= '<tr><td><b id="textmatter">Street Address</b></td><td>'.tep_draw_input_field('street_address') . '&nbsp;' . (tep_not_null(ENTRY_STREET_ADDRESS_TEXT) ? '<span class="inputRequirement">' . ENTRY_STREET_ADDRESS_TEXT . '</span>': '').'</td></tr>';
   }
  }
   if(empty($lrdata['Email'])){
   $output .= '<tr><td><b id="textmatter">Email</b></td><td>'.tep_draw_input_field('email_address') . '&nbsp;' . (tep_not_null(ENTRY_EMAIL_ADDRESS_TEXT) ? '<span class="inputRequirement">' . ENTRY_EMAIL_ADDRESS_TEXT . '</span>': '').'</td></tr>';
   }
   if($lrsetting['allfields'] != 0){
   if(empty($lrdata['city']) || $lrdata['city'] =='unknown'){
   $output .= '<tr><td><b id="textmatter">City</b></td><td>'.tep_draw_input_field('city') . '&nbsp;' . (tep_not_null(ENTRY_CITY_TEXT) ? '<span class="inputRequirement">' . ENTRY_CITY_TEXT . '</span>': '').'</td></tr>';
   }
   if(empty($lrdata['state'])){
   $output .= '<tr><td><b id="textmatter">State/Province</b></td><td>';
      if (isset($_POST['country']) && !empty($_POST['country'])) {
       
        $zones_array = array();
        $zones_query = tep_db_query("select zone_name from " . TABLE_ZONES . " where zone_country_id = '" . (int)$_POST['country'] . "' order by zone_name");
		$zones = tep_db_fetch_array($zones_query);
		if ($zones) {
        while ($zones_values = tep_db_fetch_array($zones_query)) {
          $zones_array[] = array('id' => $zones_values['zone_name'], 'text' => $zones_values['zone_name']);
        }
        $output .= tep_draw_pull_down_menu('state', $zones_array);
      } else {
...
$output .='</td></tr>';
   ...
   $output .= '<tr><td><b id="textmatter">Country</b></td><td>'.tep_get_country_list('country') . '&nbsp;' . (tep_not_null(ENTRY_COUNTRY_TEXT) ? '<span class="inputRequirement">' . ENTRY_COUNTRY_TEXT . '</span>': '').'</td></tr>';
   }   
   
   if(empty($lrdata['postcode'])){
   $output .= '<tr><td><b id="textmatter">Post/Zip Code</b></td><td>'.tep_draw_input_field('postcode') . '&nbsp;' . (tep_not_null(ENTRY_POST_CODE_TEXT) ? '<span class="inputRequirement">' . ENTRY_POST_CODE_TEXT . '</span>': '').'</td></tr>';
   }
   if(empty($lrdata['telephone'])){
   $output .= '<tr><td><b id="textmatter">Telephone</b></td><td>'.tep_draw_input_field('telephone') . '&nbsp;' . (tep_not_null(ENTRY_TELEPHONE_NUMBER_TEXT) ? '<span class="inputRequirement">' . ENTRY_TELEPHONE_NUMBER_TEXT . '</span>': '').'</td></tr>';
   }
   $output .= '</table><div>'; 
   //$output .= tep_get_country_list('country') . '&nbsp;' . (tep_not_null(ENTRY_COUNTRY_TEXT) ? '<span class="inputRequirement">' . ENTRY_COUNTRY_TEXT . '</span>': '');
